/*  =========================================================================
    zm_device_selftest.c - run selftests
...
{
    test_item_t *item;
    printf ("Running zm-device selftests...\n");
    for (item = all_tests; item->test; item++)
        item->test (verbose);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("zm_device_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running zm-device test '%s'...\n", test->testname);
        test->test (verbose);
    }
    else
        test_runall (verbose);
